...
pub mod proto {
    tonic::include_proto!("upvpn.controller");
}
...
pub async fn new_grpc_client() -> Result<ControllerServiceClient, ControllerError> {
    let ipc_path = upvpn_config::config().socket_path();
    ...
    P: Auth + 'static,
{
    ...
    let socket_path = upvpn_config::config().socket_path();
