...
fun VPNApp(
    ...
    // trigger in-app review when eligible
    val shouldRequestReview = homeVM.shouldRequestReview.collectAsStateWithLifecycle()
    LaunchedEffect(shouldRequestReview.value) {
        if (shouldRequestReview.value) {
            homeVM.ackReviewRequest()
            (context as? Activity)?.let { activity ->
                homeVM.launchReviewFlow(activity)
            }
        }
    }
