...
impl VpnSessionHandler {
    ...
    pub async fn new_session(
        &self,
        new_session: upvpn_types::upvpn_server::NewSession,
    ...
    pub async fn end_session(
        &self,
        end_session: upvpn_types::upvpn_server::EndSession,
...
impl<P: TokenProvider + 'static> VpnSessionService<P> {
    ...
    async fn on_new_session_inner(
        ...
        Ok(upvpn_service.new_session(new_session).await?)
    }
    ...
    async fn on_client_connected_inner(
        ...
        let mut upvpn_service = ServerApi::new(token_provider).await?;
        Ok(upvpn_service.client_connected(client_connected).await?)
    }
    ...
    async fn on_list_locations_inner(
        ...
        let mut upvpn_service = ServerApi::new(token_provider).await?;
        let locations = upvpn_service.list_locations().await?;
        Ok(locations)
    }
